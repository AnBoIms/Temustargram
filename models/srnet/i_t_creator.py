...
# 폴더 경로 설정
input_folder = "static/cropped"  # PNG 파일이 있는 폴더 경로
output_folder = "custom_feed/labels/"  # 회색 배경 이미지를 저장할 폴더 경로
...
def save_original_image(img, output_path):
    ...
    print(f"원본 이미지 저장 완료: {output_path}")
...
def process_all_images(input_folder, output_folder, text):
    ...
    for file_name in os.listdir(input_folder):
        if file_name.lower().endswith(".png"):  # 대소문자 구분 없이 PNG 파일 확인
            ...
            # 이름에서 _sign 제거
            cleaned_name = file_name.replace("_sign", "")
